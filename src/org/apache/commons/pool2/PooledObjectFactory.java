 ...
 * from the pool. {@link #validateObject} may also be used to test an instance
 * being {@link ObjectPool#returnObject returned} to the pool before it is
 * {@link #passivateObject passivated}. It will only be invoked on an activated
 * instance.</li>
 * <li>
 * {@link #passivateObject} is invoked on every instance when it is returned to
 * the pool.</li>
 * <li>
 * {@link #destroyObject} is invoked on every instance when it is being
 * "dropped" from the pool (whether due to the response from
 * {@link #validateObject}, or for reasons specific to the pool implementation.)
 * There is no guarantee that the instance being destroyed will be considered
 * active, passive or in a generally consistent state.</li>
 * </ol>
 * {@link PooledObjectFactory} must be thread-safe. The only promise an
 * {@link ObjectPool} makes is that the same instance of an object will not be
 * passed to more than one method of a <code>PoolableObjectFactory</code> at a
 * time.
 * <p>
 ...
 * that pools use to track and maintain state information about the objects that
 * they manage.
 * 
 * @param <T>
...
public interface PooledObjectFactory<T>
{
	 ...
	 * 
	 * @throws Exception
	 *             should be avoided as it may be swallowed by the pool
	 *             implementation.
	 * 
	 * @see #validateObject
	 ...
	 * 
	 * @param p
	 *            a {@code PooledObject} wrapping the instance to be activated
	 * 
	 * @throws Exception
