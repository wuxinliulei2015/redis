...
public abstract class BaseGenericObjectPool<T>
{
	 ...
	 *            Pool configuration
	 * @param jmxNameBase
	 *            The default base JMX name for the new pool unless overridden
	 *            by the config
	 * @param jmxNamePrefix
	 *            Prefix to be used for JMX name for the new pool
	 */
	 ...
	 * negative, there is no limit to the number of objects that can be managed
	 ...
	 *            <code>true</code> if <code>borrowObject()</code> should block
	 *            when the pool is exhausted
	 * 
	 ...
	 * exception when the pool is exhausted and {@link #getBlockWhenExhausted}
	 * is true. When less than 0, the <code>borrowObject()</code> method may
	 * block indefinitely.
	 * 
	 ...
	 * 
	 * @param maxWaitMillis
	 *            the maximum number of milliseconds <code>borrowObject()</code>
	 ...
	 * fairly. True means that waiting threads are served as if waiting in a
	 * FIFO queue.
	 * 
	 * @return <code>true</code> if waiting threads are to be served by the pool
	 *         in arrival order
	 */
	...
	/**
	 * Sets whether objects created for the pool will be validated before being
	 * returned from the <code>borrowObject()</code> method. Validation is
	 * performed by the <code>validateObject()</code> method of the factory
	 * associated with the pool. If the object fails to validate, then
	 * <code>borrowObject()</code> will fail.
	 * 
	 * @param testOnCreate
	 *            <code>true</code> if newly created objects should be validated
	 *            before being returned from the <code>borrowObject()</code>
	 *            method
	 * 
	 * @see #getTestOnCreate
	 * 
	 ...
	 *            returned from the <code>borrowObject()</code> method
	 * 
	 * @see #getTestOnBorrow
	 */
	 ...
	 * 
	 * @return <code>true</code> if objects are validated on return to the pool
	 *         via the <code>returnObject()</code> method
	 * 
	 * @see #setTestOnReturn
	 */
	...
	/**
	 * Sets whether objects borrowed from the pool will be validated when they
	 * are returned to the pool via the <code>returnObject()</code> method.
	 * Validation is performed by the <code>validateObject()</code> method of
	 * the factory associated with the pool. Returning objects that fail
	 * validation are destroyed rather then being returned the pool.
	 * 
	 * @param testOnReturn
	 *            <code>true</code> if objects are validated on return to the
	 *            pool via the <code>returnObject()</code> method
	 * 
	 * @see #getTestOnReturn
	 */
	...
	/**
	 * Sets the maximum number of objects to examine during each run (if any) of
	 * the idle object evictor thread. When positive, the number of tests
	 * performed for a run will be the minimum of the configured value and the
	 * number of idle instances in the pool. When negative, the number of tests
	 * performed will be <code>ceil({@link #getNumIdle}/
	 * abs({@link #getNumTestsPerEvictionRun}))</code> which means that when the
	 ...
	 * it is eligible for eviction by the idle object evictor (if any - see
	 * {@link #setTimeBetweenEvictionRunsMillis(long)}). When non-positive, no
	 ...
	 * Returns the minimum amount of time an object may sit idle in the pool
	 * before it is eligible for eviction by the idle object evictor (if any -
	 * see {@link #setTimeBetweenEvictionRunsMillis(long)}), with the extra
	 * condition that at least <code>minIdle</code> object instances remain in
	 * the pool. This setting is overridden by
	 ...
	 * Sets the minimum amount of time an object may sit idle in the pool before
	 * it is eligible for eviction by the idle object evictor (if any - see
	 * {@link #setTimeBetweenEvictionRunsMillis(long)}), with the extra
	 * condition that at least <code>minIdle</code> object instances remain in
	 * the pool. This setting is overridden by
	 * {@link #getMinEvictableIdleTimeMillis} (that is, if
	 * {@link #getMinEvictableIdleTimeMillis} is positive, then
	...
	public final void setEvictionPolicyClassName(String evictionPolicyClassName)
	{
		try
		{
			Class<?> clazz = Class.forName(evictionPolicyClassName, true, Thread.currentThread().getContextClassLoader());
	...
	/**
	 * <p>
	 * Starts the evictor with the given delay. If there is an evictor running
	 * when this method is called, it is stopped and replaced with a new evictor
	 * with the specified delay.
	 * </p>
	 * 
	 * <p>
	 * This method needs to be final, since it is called from a constructor. See
	 * POOL-195.
	 * </p>
	 * 
	 * @param delay
	 *            time in milliseconds before start and between eviction runs
	 */
	 ...
	 *            The listener or <code>null</code> for no listener
	 */
	 ...
	 * 
	 * @param activeTime
	 *            the amount of time (in milliseconds) that the returning object
	 *            was checked out
	 */
	...
	/**
	 * Registers the pool with the platform MBean server. The registered name
	 * will be <code>jmxNameBase + jmxNamePrefix + i</code> where i is the least
	...
	class Evictor extends TimerTask
	{
		 ...
		 * ensure that the minimum number of idle instances are available. Since
		 * the Timer that invokes Evictors is shared for all Pools but pools may
		 * exist in different class loaders, the Evictor ensures that any
		 * actions taken are under the class loader of the factory associated
		 * with the pool.
		 */
		@Override
		public void run()
	...
	/**
	 * Maintains a cache of values for a single metric and reports statistics on
	 * the cached values.
	 */
	private class StatsStore
	{
		...
		/**
		 * Adds a value to the cache. If the cache is full, one of the existing
		 * values is replaced by the new value.
		 * 
		 * @param value
