...
/**
 * 管道的最顶层接口,提供一系列reids操作命令接口.参数均为byte数组形式
 */
public interface BinaryRedisPipeline
{
	...
	Response<Set<byte[]>> zrangeByLex(final byte[] key, final byte[] min, final byte[] max, int offset, int count);

	Response<Set<byte[]>> zrevrangeByLex(final byte[] key, final byte[] max, final byte[] min);

	Response<Set<byte[]>> zrevrangeByLex(final byte[] key, final byte[] max, final byte[] min, int offset, int count);
