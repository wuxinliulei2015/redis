...
public class BinaryClient extends Connection
{
	...
	/**
	 * 创建Jedis对象时会调用(此时会真正去进行connect) <br>
	 * 发送命令前会调用,用于检测链接是否可用,不可用则重新建立链接
	 */
	@Override
	public void connect()
	{
	...
	{
		// Note that it actually calls ZADD with INCR option, so it requires
		// Redis 3.0.2 or upper.
		sendCommand(ZADD, params.getByteParams(key, toByteArray(score), member));
	}
	...
	/**
	 * 发送discard命令,同时将isInMulti和isInWatch置为false
	 */
	public void discard()
	{
		sendCommand(DISCARD);
	...
	/**
	 * 发送exec命令,同时将isInMulti和isInWatch置为false
	 */
	public void exec()
	{
		sendCommand(EXEC);
