		...
		BinaryScriptingCommands, Closeable
{
	 ...
	 * 
	 * @see <a
	 *      href="http://code.google.com/p/redis/wiki/ExpireCommand">ExpireCommand</a>
	 * @param key
	 * @param seconds
	...
	/**
	 * @deprecated use BinaryJedis.pexpire(byte[], long) or
	 *             Jedis.pexpire(String,long) Set a timeout on the specified
	 *             key. After the timeout the key will be automatically deleted
	 *             by the server. A key with an associated timeout is said to be
	 *             volatile in Redis terminology.
	 *             <p>
	 *             Voltile keys are stored on disk like the other keys, the
	 *             timeout is persistent too like all the other aspects of the
	 *             dataset. Saving a dataset containing expires and stopping the
	 *             server does not stop the flow of time as Redis stores on disk
	 *             the time when the key will no longer be available as Unix
	 *             time, and not the remaining milliseconds.
	 *             <p>
	 *             Since Redis 2.1.3 you can update the value of the timeout of
	 *             a key already having an expire set. It is also possible to
	 *             undo the expire at all turning the key into a normal key
	 *             using the {@link #persist(byte[]) PERSIST} command.
	 *             <p>
	 *             Time complexity: O(1)
